...
interface GameGridProps {
  ...
  /** Omit for a read-only grid (e.g. Home's "recent games" preview) - only
   * pass this where deleting a game should actually be offered. */
  onDeleteJob?: (jobId: string) => void;
  /** Omit to leave out the "add game" tile entirely. */
  onAddVideo?: () => void;
  /** Whether "add game" renders as a dashed tile inside the grid itself
   ...
   * instead (see GamesPage). Only matters when onAddVideo is given.
   * Defaults to true. */
  showAddTile?: boolean;
}
    ...
    <Card variant="outlined" sx={{ overflow: "hidden", borderStyle: "dashed" }}>
      <CardActionArea onClick={onAddVideo}>
        ...
        >
          ...
          <Typography sx={{ fontWeight: 600 }}>Add game</Typography>
        </Box>
      </CardActionArea>
    </Card>
  );
...
// Shared by HomePage's "recent games" preview and the full GamesPage grid
...
// "Add game" is a tile in the grid itself (always first) rather than a
...
export function GameGrid({ jobs, onSelectJob, onDeleteJob, onAddVideo, showAddTile = true }: GameGridProps) {
  ...
  if (jobs.length === 0 && !onAddVideo) {
    return <Typography color="text.secondary">No games yet - add one to get started.</Typography>;
  }
            ...
            <Card variant="outlined" sx={{ position: "relative", overflow: "hidden" }}>
              ...
              {job.winner_team_name && (
                <Chip
                  ...
                  label={
                    job.loser_team_name
                      ? `${job.winner_team_name} def. ${job.loser_team_name}`
                      : `${job.winner_team_name} Win`
                  }
                  sx={{
                    position: "absolute",
                    top: 6,
