...
from Backend.MachineLearning.machineLearningCommon import download_roboflow_project  # noqa: E402

IMAGE_EXTENSIONS = {".jpg", ".jpeg", ".png"}


def _find_images(source: Path) -> list:
    """Every image in a YOLO-format dataset folder (Roboflow's own
    train/valid/test split, each with an images/+labels/ pair) - shared by
    BallDatasets and ActionDatasets, both of which merge YOLO detection
    data directly."""
    ...
    def merge_all(sources: list, out_name: str = "merged", val_fraction: float = 0.1) -> Path:
        ...
        for source in sources:
            ...
            if ball_index is None:
                ...
                continue
            for image_path in _find_images(source):
                label_path = _matching_label(image_path)
...
class ActionDatasets:
    """Action-detector training data sources, used by
    ActionDetection/actionDetection.py's full-frame detector once trained.

    Every source below is already a YOLO-format object-detection export
    (train/valid/test, each with images/+labels/, plus one data.yaml at the
    root listing class names) - merge_all combines them directly, the same
    way BallDatasets.merge_all does, remapping each source's own class
    vocabulary onto this app's 4-class scheme (APP_ACTION_CLASSES below) via
    CLASS_MAPS. Unlike the ball detector's single-class remap, an image can
    keep several of its original boxes at once (e.g. both a "set" and a
    "spike" box), each remapped independently.
    ...
    left out, see below):
        ...
        - activity-graz-uni/volleyball-activity       CC BY 4.0   ~25k images (Roboflow
          ...
          if redistributing a model trained on it)
    ...
    Deliberately EXCLUDED despite showing up in the same search:
        - actions-players/volleyball-actions: license badge conflicts with a
          ...
          dataset - training on it would itself be a derivative, so this is
        ...
        - Ibrahim et al. Volleyball Dataset (CVPR'16): citation-required, no
          explicit commercial grant, and its per-player tracking-annotation
          format isn't YOLO-shaped - would need a one-off conversion step
          this project doesn't carry for a source this low-confidence on
          licensing anyway.
    ...
    # This app's own action-type vocabulary, in the fixed order written into
    # merged data.yaml files below (and therefore into a trained detector's
    # own class-index<->name mapping) - "serve" isn't here because it's
    # detected separately by actionDetection.py's own timing rule, not this
    # detector.
    APP_ACTION_CLASSES = ["block", "dig", "set", "spike"]
    ...
    # handled by actionDetection.py's own timing rule, not this detector,
    # and non-action classes obviously don't belong here.
    CLASS_MAPS = {
        "mikhail_klyukin": {
    ...
    @staticmethod
    def _class_index_map(source_dir: Path, source_name: str) -> dict:
        """Reads source_dir/data.yaml's own class names and maps each one
        that CLASS_MAPS[source_name] recognizes onto its index in
        APP_ACTION_CLASSES - e.g. {2: 3} if this source's class 2 is
        "spiking" and APP_ACTION_CLASSES.index("spike") == 3. Classes not
        listed in CLASS_MAPS (non-action classes, or "serve" variants) are
        simply absent from the returned map, so their boxes get dropped by
        _remap_multi_label_lines below."""
        class_map = ActionDatasets.CLASS_MAPS.get(source_name)
        ...
        index_map = {}
        for index, name in enumerate(names):
            app_class = class_map.get(str(name).strip().lower())
            if app_class is not None:
                index_map[index] = ActionDatasets.APP_ACTION_CLASSES.index(app_class)
        return index_map

    @staticmethod
    def _remap_multi_label_lines(label_path: Path, index_map: dict) -> Optional[str]:
        """Keeps every line of this label file whose class is a key in
        index_map (unlike BallDatasets' single-class remap, several of a
        source's own classes can each map onto a different one of this
        app's 4 classes), rewritten with the remapped index. Drops every
        other line (non-action classes, e.g. "serve"/"ball"/"net")."""
        lines_out = []
        for line in label_path.read_text().splitlines():
            parts = line.split()
            if not parts:
                continue
            new_index = index_map.get(int(parts[0]))
            if new_index is None:
                continue
            lines_out.append(" ".join([str(new_index), *parts[1:]]))
        return "\n".join(lines_out) + "\n" if lines_out else None

    @staticmethod
    def merge_all(sources: list, out_name: str = "merged", val_fraction: float = 0.15) -> Path:
        """Combines every dataset folder gathered above into one YOLO
        detection dataset ready for training - remaps each source's own
        class vocabulary onto APP_ACTION_CLASSES (dropping any other class,
        and any label line/image that has no action class left after that
        filter), prefixes filenames by source folder name to avoid
        collisions between sources, and writes one shuffled train/val split
        + data.yaml at DATASETS_DIR/out_name. Always rebuilds that folder
        from scratch rather than appending, matching BallDatasets.merge_all's
        own "never leave stale files" behaviour.
        ...
        pairs = []  # (image_path, label_path, index_map, prefix)
        for source in sources:
            index_map = ActionDatasets._class_index_map(source, source.name)
            if not index_map:
                print(f"[{source.name}] couldn't map any of its classes onto {ActionDatasets.APP_ACTION_CLASSES} "
                      f"(missing/unrecognized data.yaml, or no CLASS_MAPS entry) - skipping this source.")
                continue
            for image_path in _find_images(source):
                label_path = _matching_label(image_path)
                if label_path.exists():
                    pairs.append((image_path, label_path, index_map, source.name))
        ...
        written = 0
        for i, (image_path, label_path, index_map, prefix) in enumerate(pairs):
            images_dir, labels_dir = (train_images, train_labels) if i < split_index else (val_images, val_labels)
            remapped = ActionDatasets._remap_multi_label_lines(label_path, index_map)
            if remapped is None:
                continue
            stem = f"{prefix}_{image_path.stem}"
            shutil.copy2(image_path, images_dir / f"{stem}{image_path.suffix}")
        ...
        data_yaml = out_dir / "data.yaml"
        names_literal = ", ".join(f"'{c}'" for c in ActionDatasets.APP_ACTION_CLASSES)
        data_yaml.write_text(
            f"train: train/images\nval: val/images\nnc: {len(ActionDatasets.APP_ACTION_CLASSES)}\nnames: [{names_literal}]\n"
        )
        print(f"Merged {written} labeled image(s) from {len(sources)} source(s) into {out_dir}")
        return data_yaml
...
class CourtDatasets:
    ...
    CourtDetection/court.py's keypoint model once fine-tuned.

    Like BallDatasets/ActionDatasets, this merges YOLO-format sources
    directly rather than cropping - a keypoint model needs the WHOLE image
    plus its keypoint annotations. This class's job is narrower though:
    download each source, remap its own keypoint vocabulary onto this project's 4-point
    ...
    DATASETS_DIR = ML_DIR / "dataset_courtDetection"
