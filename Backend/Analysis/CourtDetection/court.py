...
# ends of the OTHER marked baseline - rather than the desktop tool's full
# 6-point set above (4 corners + 2 net-top). Those 4 points alone are enough
# to solve a full pixel<->court homography (cv2.getPerspectiveTransform needs
# exactly 4 correspondences), and once solved, every other line on the court
# - the far baseline and both attack lines - has a *known* real-world
# position (see the layout below), so where it lands in the image is a
# matter of projecting through the *inverse* of that same homography, not
# something the user has to mark by hand. Verified against a synthetic
# ground-truth perspective transform to sub-pixel accuracy.
#
# NOTE on naming: the parameter/field name "far_left"/"far_right" (here and
# throughout create_half_court_homography, estimate_camera_pose, calibration
# .py, and the API/frontend schema) is kept for compatibility, but it's
# actually the NEAR baseline (x=0, closest to camera) - CalibrationPanel.tsx
# labels this marker "Close Line Left/Right" on the live canvas, and that
# label is what a real person calibrating actually follows, so the marked
# pixel is the close/near one. This is why the destination/world point below
# maps far_left/far_right to x=0.0, not COURT_LENGTH.
#
#   x=0 (marked "far_left/right",   x=6 (near     x=9 (net/     x=12 (far     x=18 (predicted
#        actually the near end)      attack)       middle)       attack)        far baseline)
#        |------------------|--------------|------------------|------------------|
...
HALF_COURT_DESTINATION = np.array([
    ...
    [0.0, 0.0],                        # far_left  (see NOTE above - actually the near baseline)
    [0.0, COURT_WIDTH],                # far_right
], dtype=np.float32)
...
def create_half_court_homography(middle_left, middle_right, far_left, far_right):
    ...
    and the marked "far" (actually near - see module NOTE above) baseline.
    Argument order matters: it has to line up with HALF_COURT_DESTINATION."""
...
def camera_pose_candidates(
    ...
    world_points = np.array([
        ...
        [0.0, 0.0, 0.0],                          # far_left  (see court.py's NOTE on naming above)
        [0.0, COURT_WIDTH, 0.0],                  # far_right
...
def predict_court_geometry(matrix):
    ...
    Returns pixel coordinates (as (x, y) tuples) for the far baseline's two
    corners and both attack lines' sideline-to-sideline endpoints. (The
    marked baseline - see create_half_court_homography's NOTE - is actually
    the near one, x=0, so what's left to predict is the far one, x=18.)"""
    ...
    return {
        "far_left": to_pixel(far_x, 0.0),
        "far_right": to_pixel(far_x, COURT_WIDTH),
...
def detectCourt(video_path,output_path):
