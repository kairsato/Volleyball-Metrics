...
# The stage modules live under Backend/Analysis/, a sibling of the API/ package.
...
from CourtDetection.court import (  # noqa: E402
    CALIBRATION_FRAME,
    COURT_LENGTH,
    COURT_WIDTH,
...
# CourtDetection.court's own NET_PRESET_HEIGHT_PX so the desktop and web
...
def default_points(frame_width: int, frame_height: int) -> dict:
    ...
    points, shaped like a typical sideline view: the "far_left"/"far_right"
    marker (actually the near baseline - CalibrationPanel.tsx labels it
    "Close Line Left/Right" on the live canvas, and that's the label a real
    person actually follows; see CourtDetection.court's NOTE on this naming)
    sits lower and wider than the net line, closer to the camera - purely a
    starting position to drag from, not a real estimate of this particular
    shot's perspective. The 4 ground points are chosen (via a grid search
    against predict_court_geometry) so the predicted far baseline and both
    attack lines land within the visible frame for this starting shape - a
    preset with a much steeper near/close ratio pushes the predicted far
    baseline far outside the frame before the user has even touched a
    point. The 2 net-top points start directly above middle_left/
    middle_right (NET_TOP_PRESET_OFFSET_PX higher) - dragging them onto the
    actual net/antenna top is what net_top_calibrated (see save() below)
    checks for."""
    close_top = int(frame_height * 0.80)
    close_left_x = int(frame_width * 0.10)
    close_right_x = int(frame_width * 0.90)
    ...
    return {
        ...
        "far_left": {"x": close_left_x, "y": close_top},
        "far_right": {"x": close_right_x, "y": close_top},
...
def existing_points(output_path: Path, frame_width: int, frame_height: int) -> Optional[dict]:
    ...
    # points - see CourtDetection.court's desktop tool, still the format
...
def save(
    ...
    data = {
        ...
        # Purely for the frontend to draw as a preview - the far baseline
        ...
        # A full, sequential 4-corner quad (marked "far"/actually-near
        # baseline + predicted far baseline) - kept in this exact shape for
        # ball_detection.py's court-area restriction (load_court_corners),
        # which just wants a simple polygon to test points against and
        # doesn't care how those corners were actually obtained.
        "image_points": [
            {"x": far_left[0], "y": far_left[1]},
            {"x": far_right[0], "y": far_right[1]},
            {"x": predicted["far_right"][0], "y": predicted["far_right"][1]},
            {"x": predicted["far_left"][0], "y": predicted["far_left"][1]},
        ],
