...
def rebase_actions(actions: list[dict], start_s: float, end_s: float) -> list[dict]:
    """Same idea as rebase_rallies, for a raw actions.json list instead of
    game_status.json's rallies - drops anything outside [start_s, end_s] and
    shifts the rest to be relative to start_s. Needed by
    Backend/API/segment_merge.py, which trims each segment's warmup range
    against its own raw actions.json BEFORE that segment's contribution to
    the merged job-level timeline is computed - consolidateStats (which
    rebase_stats mirrors) hasn't run yet at that point, so there's no
    player_stats.json to rebase instead."""
    kept = []
    for action in actions:
        if not _in_range(action["timestamp_s"], start_s, end_s):
            continue
        kept.append({**action, "timestamp_s": action["timestamp_s"] - start_s})
    return kept
