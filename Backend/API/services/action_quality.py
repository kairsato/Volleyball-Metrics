...
from . import teams
from .players import load_player_positions
...
# Half of CourtDetection.court.COURT_LENGTH - duplicated here rather than
...
BLOCK_WEIGHTS = {"positioning": 0.35, "reach": 0.35, "timing": 0.30}
...
BLOCK_POSITION_REFERENCE_M = 1.5
BLOCK_REACH_REFERENCE_M = 3.0
BLOCK_REACTION_EXCESS_REFERENCE_S = 0.3
...
CAVEATS = [
    ...
    "Block's \"reach\" factor is the ball's own apex height at the moment "
    "of the block, as a stand-in for how high the blocker actually reached "
    "- not a real jump-height measurement. \"Timing\" reuses the same "
    "excess-reaction-time heuristic as Receive, applied to a block's own "
    "response to the preceding attack.",
]
...
def _players_by_frame(output_path: Path) -> dict[int, list[dict]]:
    # load_player_positions is cached (keyed on the file's mtime) - this
    # endpoint used to re-read and re-parse this job's own (100-250+ MB)
    # player_positions.json from scratch on every request, on top of the
    # same file already being re-read by teams.assign_teams below.
    return {entry["frame_idx"]: entry["players"] for entry in load_player_positions(output_path)}
...
def _score_spike(action, next_action, players_by_frame, team, team_by_id) -> dict:
    ...
    if team in ("A", "B"):
        ...
        if frame_players is not None:
            ...
            # Distance from the net is a ground measurement, so this uses
            # court_ground (the foot point) rather than the box-centre
            # `court`, whose elevation bias runs along this very axis on a
            # camera aimed down the court - see teams.assign_teams. Unlike
            # the placement factor above, nothing here is compared against
            # the (airborne) ball, so there is no matching bias to preserve.
            blocker_count = sum(
                1 for p in frame_players
                if (p.get("court_ground") or p.get("court")) is not None
                and team_by_id.get(p["stable_id"]) == opposing
                and abs((p.get("court_ground") or p["court"])[0] - NET_X_M) <= BLOCKER_PROXIMITY_M
            )
...
def _score_block(action, prev_action, team, frames, heights) -> dict:
    factors: dict[str, Optional[float]] = {}

    court = action.get("ball_court")
    if court is not None:
        distance_from_net = abs(court[0] - NET_X_M)
        factors["positioning"] = _clamp01(1 - distance_from_net / BLOCK_POSITION_REFERENCE_M)
    else:
        factors["positioning"] = None

    # Ball's own apex height at the block's frame stands in for how high the
    # blocker reached - see CAVEATS.
    apex = _apex_height(action["frame_idx"], action["frame_idx"], frames, heights)
    factors["reach"] = _clamp01(apex / BLOCK_REACH_REFERENCE_M) if apex is not None else None

    # Same excess-reaction-time heuristic as _score_receive's "reaction"
    # factor, applied to how promptly the block met the preceding attack.
    factors["timing"] = None
    speed_in = action.get("speed_in_m_per_s")
    gap = action.get("time_since_prev_touch_s")
    if prev_action is not None and speed_in and gap is not None \
        ...
        expected_s = travel_distance / speed_in
        excess_s = max(0.0, gap - expected_s)
        factors["timing"] = _clamp01(1 - excess_s / BLOCK_REACTION_EXCESS_REFERENCE_S)

    return _instance(action, team, factors, BLOCK_WEIGHTS)
...
def compute_action_quality(job_id: str, output_path: Path) -> dict:
    ...
    serve_instances, receive_instances, set_instances, spike_instances, block_instances = [], [], [], [], []
    ...
    for action in ordered:
        ...
        if action_type == "serve":
            ...
            block_instances.append(_score_block(action, prev_action, team, frames, heights))
    ...
    return {
        ...
        "block": {"weights": BLOCK_WEIGHTS, "reference": {}, **_summarize(block_instances)},
        "caveats": CAVEATS,
    }
