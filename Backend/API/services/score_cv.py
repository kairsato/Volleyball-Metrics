...
each set boundary sits, purely from *when a side's digit changes* -
...
ball-out-of-bounds heuristic - everything downstream (set grouping, then
...
def _existing_baseline_before(existing: Optional[dict], rally_index: int) -> tuple[int, int]:
    ...
    range-scoped re-run pick up mid-set without needing to re-read
    anything before the range. Resets to (0, 0) at the start of whichever
    set rally_index falls in, exactly like the main scoring loop below
    does on a real set-break reading."""
    if existing is None:
        return (0, 0)

    set_index = next(
        (s["set_index"] for s in existing["sets"] if s["start_rally_index"] <= rally_index <= s["end_rally_index"]),
        None,
    )
...
def compute_cv(output_path: Path, video_path: Optional[Path], rally_range: Optional[tuple[int, int]] = None) -> dict:
    ...
    narrow range (e.g. "Sets 3-4" out of ten) used to still sample every
    ...
    # Which geometric side (see attribute_sides_per_set) the left-read
    ...
    for rally in target_rallies:
        ...
        # set started somewhere before this rally - the physical/digital
    ...
    if rally_range is None:
        ...
        lo, hi = rally_range
        existing_boundaries = {s["end_rally_index"] for s in existing["sets"][:-1]} if existing and existing["sets"] else set()
    ...
    sets = score.build_sets_from_boundaries(rallies, merged_split_after)
    attribution = score.attribute_sides_per_set(output_path, rallies, sets, cfg["team_x_id"], cfg["team_y_id"])
    ...
    for rally in rallies:
        ...
        set_index = set_by_rally[rally_index]
        ...
        if not in_range and rally_index in existing_by_rally:
            ...
            result_rallies.append({**previous, "set_index": set_index})
            continue

        ab_winner = winners.get(rally_index)
        set_attribution = attribution.get(set_index, {})
        winner = set_attribution.get(ab_winner) if ab_winner else None
    ...
    return score.save_result(output_path, {"sets": sets, "rallies": result_rallies})
